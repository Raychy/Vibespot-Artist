...
import 'package:vsartist/views/artist_features/uploads/payment-bloc.dart';
...
class _UploadPaymentState extends State<UploadPayment> {
  ...
  Future<dynamic> initiatePayment() {
    return http
        .get(Uri.parse("http://test.afiaanyi.com/api/directoryoverview"))
  ...
  Future<dynamic> verifyPayment(String? reference) {
    return http.post(
        Uri.parse("http://test.afiaanyi.com/api/me/verifypayments"),
      ...
      if (!json.isEmpty) {
        ...
        if (json != null) {
          ...
          Charge charge = Charge()
            ..amount = amount
            ..reference =
                'hgjgkjhkugjghh' //DateTime.now().millisecondsSinceEpoch.toString()
