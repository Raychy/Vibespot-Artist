...
import 'package:vsartist/views/artist_features/uploads/payment-model.dart';
...
class Paymentloc {
  ...
  Stream<PaymentDetailsModel> get getPaymentDetails => _getPaymenDetails.stream;
...
//print(UiData.domain + "/payment/details?id=$id&type=$type");
    String parsed = await network.get(Uri.encodeFull(
        UiData.domain + "/a1/payment/details?id=$id&type=$type"));
    print(parsed);
  ...
  fetchNewPayCode(id, type) async {
    ...
    String parsed = await network.get(Uri.encodeFull(
        UiData.domain + "/a1/payment/new-payment-code?id=$id&type=$type"));
  ...
  updateServerPayment(id, type, reference, message, option) async {
    ...
    String parsed = await network.get(Uri.encodeFull(UiData.domain +
        "/a1/payment/update-payment?id=$id&type=$type&ref=$reference&message=$message&source=$platform&option=$option"));
    var response = jsonDecode(parsed);
    if (response["status"] == false) {
      snackBar.add('Could not fetch data');
      throw new Exception("Error while fetching data");
