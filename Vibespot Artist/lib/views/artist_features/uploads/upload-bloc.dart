...
class UploadBloc {
  ...
  Stream<Music> get profile => _uploadSingles.stream;
  ...
  uploadSongs(UploadSingles upload, context) async {
    ...
    var response = await network.post(UiData.domain + "/a1/songs/new-release",
    ...
    if (res['status']) {
      ...
      for (var i = 0; i < upload.tracksNumber!; i++) {
        ...
        String response = await (network.postWithFile(
            UiData.domain + "/a1/songs/upload",
            body: datas) as FutureOr<String>);
        print(response);
  ...
  uploadAlbum(UploadAlbum upload, context) async {
    List<int>? charCodes = upload.single_Ids;
    ...
    var response = await network.post(UiData.domain + "/a1/songs/new-album",
        body: newAlbum.toJson(newAlbum), context: context);
    var res = jsonDecode(response);
    print('1');
    if (res['status']) {
      UploadAlbumDetails release = UploadAlbumDetails.fromJson(res['body']);
      if (upload.single_Ids != null) {
        Map<String, dynamic> _buildParameters() {
          return {
            'album_id': release.albumId.toString(),
            'single_ids': charCodes,
          };
        }

        FormData datas = new FormData.fromMap(_buildParameters());
        String response = await (network.postWithForms(
            UiData.domain + "/a1/songs/single-to-album",
            body: datas) as FutureOr<String>);
        if (response == "200") {
          print('3');
          for (var i = 0; i < upload.trackCount!; i++) {
            if (upload.tracks![i] != null && upload.tracks![i].title != null) {
              upload.tracks![i].albumId = release.albumId.toString();
              upload.tracks![i].release_date = release.releaseDate.toString();
              print(music.toJson(upload.tracks![i]));
              FormData datas =
                  new FormData.fromMap(music.toJson(upload.tracks![i]));
              String response = await (network.postWithFile(
                  UiData.domain + "/a1/songs/album-tracks",
                  body: datas) as FutureOr<String>);
